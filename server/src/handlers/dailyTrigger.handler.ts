...
export async function handleDailyTrigger(job: Job) {
  ...
  console.log(
    `[daily-trigger] Found ${filters.length} scrape filters to process`,
  );
  ...
  for (const filter of filters) {
    try {
      ...
      console.error(
        `[daily-trigger] Failed to enqueue scrape for user ${filter.userId}:`,
        err,
      );
    }
  }
...
function buildPostFetchOptions(filter: {
  ...
  if (filter.jobTypes.length)
    options.push(`jobTypes=${filter.jobTypes.join(',')}`);

...
function matchesFilter(
    ...
    if (filter.location && mapped.location && !mapped.location.toLowerCase().includes(filter.location.toLowerCase())) {
        return false;
    }
    if (filter.salaryMin && mapped.salaryMax !== undefined && mapped.salaryMax < filter.salaryMin) {
        return false;
    }
    if (filter.jobTypes?.length && mapped.jobType && !filter.jobTypes.includes(mapped.jobType)) {
        return false;
    }
    return true;
}
...
export async function handleScrapeJob(job: BullJob<ScrapeJobData>) {
    ...
    let saved = 0, skippedDuplicate = 0, skippedFilter = 0;
    ...
    return { totalFetched: rawJobs.length, saved, skippedDuplicate, skippedFilter };
}